  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (intensity === 0) {
        ...
        for (let x = -offset; x < width + wavelengthPixels; x += halfWavelength) {
          const nextY = isHigh ? highVoltageY : zeroY;
          ctx.lineTo(Math.max(0, Math.min(width, x)), nextY);
          isHigh = !isHigh;
          ctx.lineTo(Math.max(0, Math.min(width, x)), isHigh ? highVoltageY : zeroY);
        }
      }
    ...
    <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', fontSize: '0.8rem', fontFamily: 'var(--font-mono)' }}>
          <span style={{ color: 'var(--accent-cyan)' }}>Freq: <strong>{frequency} Hz</strong></span>
          <span style={{ color: 'var(--accent-amber)' }}>Duty: <strong>{dutyPct}%</strong> ({intensity}/255)</span>
          <span style={{ color: 'var(--accent-green)' }}>Period: <strong>{periodMs} ms</strong></span>
        </div>
      </div>

      <div style={{ position: 'relative', width: '100%', background: '#070b14', borderRadius: '8px', overflow: 'hidden', border: '1px solid #1e293b' }}>
        <canvas
          ref={canvasRef}
          width={800}
          height={240}
        ...
        <div style={{
          ...
          <div>Mode: <strong style={{ color: '#fff' }}>{frequency === 0 ? 'DC Continuous' : 'Square Wave Pulse'}</strong></div>
          <div>Voltage: <strong style={{ color: 'var(--accent-amber)' }}>{((intensity / 255) * 12).toFixed(1)} V (Peak)</strong></div>
        </div>
      </div>
    </div>
  );
