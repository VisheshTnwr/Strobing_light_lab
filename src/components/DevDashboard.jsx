...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (commaIndex > 0) {
      currentIntensity = constrain(incomingCommand.substring(0, commaIndex).toInt(), 0, 255);
      currentFrequency = incomingCommand.substring(commaIndex + 1).toFloat();
  ...
  if (currentFrequency == 0 || currentIntensity == 0) {
    ...
    if (currentMillis - previousMillis >= phaseDuration) {
      ...
      writePwm(isLedHighPhase ? currentIntensity : 0);
    }
  }
}`;
 ...
 * Signal path: USB Serial -> ESP32 -> MOSFET Gate (Pin 23)
 */
